...
async function fetchJobsPage(pageNum) {
  ...
  const url = `https://mediere.anofm.ro/api/entity/vw_public_job_posting?current=${pageNum}&rowCount=${PAGE_SIZE}&employer_tax_code=${COMPANY_CIF}`;
...
async function scrapeAllListings(testOnlyOnePage = false) {
  ...
  while (true) {
    console.log(`Fetching ANOFM page: ${page}`);
    ...
    if (page === 1) {
      ...
      console.log(`Total jobs on ANOFM: ${totalJobs}`);
    }
    let newJobs = 0;
