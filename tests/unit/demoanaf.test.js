...
const ANRAF_RECORD = {
  ...
  caenCode: '6201',
  inactive: false,
  registrationNumber: 'J40/9631/1999',
  vatRegistered: true,
...
const CACHED_DATA = {
  ...
  inactive: false,
  onrcStatusLabel: 'Funcțiune',
  administrators: [{ name: 'ANIŢEI ALEXANDRU', role: 'administrator' }],
...
describe('src/anaf.js', () => {
  ...
  describe('searchCompany', () => {
    it('should return array of companies for valid brand', async () => {
      mockFetch.mockResolvedValue(anafSearchResponse([
        { cui: 12463238, name: 'CYBERTECH SRL', statusLabel: 'Funcțiune' }
      ]));
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(anaf.searchCompany('Cybertech')).rejects.toThrow('ANAF search error: 500');
    });
    ...
    it('should encode brand name in URL', async () => {
      ...
      await anaf.searchCompany('Cybertech SRL');
      expect(capturedUrl).toContain(encodeURIComponent('Cybertech SRL'));
    });
  });
  ...
  describe('getCompanyFromANAF', () => {
    it('should return company data for valid CIF', async () => {
      ...
      const data = await anaf.getCompanyFromANAF('12463238');

      expect(data).toBeDefined();
      expect(data.cui).toBe(12463238);
      expect(data.name).toBe('CYBERTECH SRL');
    ...
    it('should throw after exhausting retries', async () => {
      ...
      await expect(anaf.getCompanyFromANAF('12463238')).rejects.toThrow();
  ...
  describe('getCompanyFromANAFWithFallback', () => {
    it('should return fresh data when API works', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('12463238');
    ...
    it('should use cached data when API fails', async () => {
      ...
      const data = await anaf.getCompanyFromANAFWithFallback('12463238', CACHED_DATA);
    ...
    it('should throw when API fails and no cache available', async () => {
      ...
      await expect(anaf.getCompanyFromANAFWithFallback('12463238')).rejects.toThrow();
    });
  });
});
