  ...
  describe('SOLR Jobs Core', () => {
    ...
    itIfSolr('should query jobs by CIF and return valid data', async () => {
      ...
      if (result.numFound === 0) {
        console.log('⚠️ No Cybertech jobs in Solr — skipping job field assertions (scraper may not have run yet)');
        return;
      }
      ...
      expect(job).toHaveProperty('company', 'CYBERTECH SRL');
  ...
  describe('Full Validation Workflow', () => {
    ...
    itIfSolr('should validate company and query SOLR for existing jobs', async () => {
      ...
      if (companyResult.existingJobsCount === 0) {
        console.log('⚠️ No Cybertech jobs in Solr — skipping job count assertion (scraper may not have run yet)');
        return;
      }
